"""Export CoronaryTemporalMTL predictions from labeled split CSVs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export predictions from a CoronaryTemporalMTL checkpoint.")
...
def main() -> int:
    ...
    if args.pretrained:
        print("Note: --pretrained is accepted for CLI consistency, but prediction reconstructs checkpoints with pretrained=False.")
    ...
    prediction_dir = resolve_evaluation_output_dir(
    ...
    print(f"Predictions CSV: {prediction_dir / 'predictions.csv'}")
    return 0
